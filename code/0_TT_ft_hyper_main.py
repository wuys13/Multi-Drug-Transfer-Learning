# 调试用：【占用内存小】调用data.get_finetune_dataloader_generator_1
import pandas as pd
import torch
import json
import os
...
def main(args, update_params_dict,to_roughly_test):
    global judge_metrics
    if args.method == 'ae':
    ...
    training_params.update(
        {
            ...
            'model_save_folder': method_save_folder, #os.path.join(method_save_folder, param_str),  一个模型就存一个unlabel model
            'es_flag': False,
    ...
    encoder, historys = train_fn(s_dataloaders=s_dataloaders, #若no-train，就根据training_params中的save_folder找到对应Pretrained_model
                                 ...
                                 iteration_method = args.iteration_method,
    ...
    for train_labeled_ccle_dataloader, test_labeled_ccle_dataloader, labeled_tcga_dataloader in labeled_dataloader_generator:
        ...
        target_classifier, ft_historys = fine_tuning1.fine_tune_encoder_drug( #有时候要这个finetune的模型，故返回target_classifier
            encoder=ft_encoder,
            ...
            to_roughly_test = to_roughly_test,
            **wrap_training_params(training_params, type='labeled')
        )
        if fold_count == 0 :
            print(f"Save decoder {param_dict}.")
            torch.save(target_classifier.decoder.state_dict(),
                        os.path.join(method_save_folder, f'decoder_store_{fold_count}.pt'))
        ...
        judge_metrics[param_str].append(ft_historys[-1]['auroc'][ft_historys[-2]['best_index']])
    ...
    if to_roughly_test:
        with open(os.path.join(training_params['model_save_folder'], 'judge_metrics_results.json'), 'w') as f:
            json.dump(judge_metrics, f)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--CCL_dataset',default = 'gdsc1_raw',
        choices=['gdsc1_raw','gdsc1_rec','gc_combine','gp_combine','gcp_combine','GDSC1_raw','GDSC1_rec'])
    parser.add_argument('--class_num',default = 0,type=int)
    parser.add_argument('--ccl_match',default = "yes",
        choices=["yes","no","match_zs"])
    parser.add_argument('--iteration_method',default = "source",
        choices=["max","min","source","target"])
    ...
    gex_features_df,CCL_tumor_type,all_ccle_gex = data.get_pretrain_dataset(
    ...
    # update_params_dict_list = update_params_dict_list[:2]  ###Just for test

    judge_metrics = defaultdict(list)
    for param_dict in update_params_dict_list:
        ...
        print('##############################################################################')   
        main(args=args, update_params_dict=param_dict,to_roughly_test=True)
    print("Finsh All !!!!!!!!!!!!!!!")

    #判断用那些param_top_n，仔细跑----
    # param_top_n = judge()
    with open(os.path.join('../results',args.store_dir, f'{args.method}_norm',
                           args.pretrain_dataset,args.tcga_construction,
                              args.select_gene_method,
                              CCL_tumor_type,args.CCL_construction,
                              tumor_type,
                              args.CCL_dataset,args.select_drug_method, 'judge_metrics_results.json'), 'r') as f:
    ...
    param_top_n = [update_params_dict_list[i] for i in top_n_index]
    print("\n\n")
    print(param_top_n)
    print("\n\n")
    ...
    param_num = 0
    judge_metrics = defaultdict(list)
    for param_dict in param_top_n:
        param_num = param_num + 1
        print(' ')
        print('##############################################################################')
        print(f'#######    Param_num {param_num}/{len(param_top_n)}       #######')
        print('Param_dict: {}'.format(param_dict) )    
        print('##############################################################################')   
        main(args=args, update_params_dict=param_dict,to_roughly_test=False)
    print("Finsh All !!!!!!!!!!!!!!!")
