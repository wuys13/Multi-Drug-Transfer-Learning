# open(os.path.join(method_save_folder,f'train_params_{drug}.json')
# 先为每个药物生成对应的参数json文件
# 默认预训练是no-train，直接根据参数得到对应的encoder文件：1.根据notrain标签读取target的dsne.pt；2.return share_encoder
# finetune重新进行
import pandas as pd
import torch
import json
