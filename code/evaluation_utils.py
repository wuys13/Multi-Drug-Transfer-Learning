...
def model_save_check_1(history, metric_name, tolerance_count=5, reset_count=1):
    save_flag = False
    stop_flag = False
    if 'best_index' not in history:
        history['best_index'] = 0
...
def evaluate_target_classification_epoch(classifier, dataloader, device, history):
    ...
    for x_smiles, x_gex , y_batch in dataloader:
...
def evaluate_target_classification_epoch_1(classifier, dataloader, device, history,class_num,test_flag = False):
    y_truths = np.array([])
    classifier.eval()
    ...
    if class_num == 0:
        ...
        for x_gex ,x_smiles, y_batch in dataloader:
            ...
            with torch.no_grad():
                y_truths = np.concatenate([y_truths, y_batch.cpu().detach().numpy().ravel()])
                # y_pred = torch.sigmoid(classifier(x_smiles,x_gex)).detach() #之前是二分类，可以直接调用sigmoid转到01区间就是，反正最后是一个rank
                y_pred = classifier(x_smiles,x_gex).detach() #多分类的时候需要可loss_fn统一，所以调用softmax得到每个分类下的概率值，但目前并不支持回归任务
                y_preds.append( y_pred.cpu().detach().tolist() )
        y_preds = [token for st in y_preds for token in st]
        y_preds = np.array(y_preds)

        # raise Exception("暂不支持回归任务,需要修改output(去掉softmax)和评估方法(PCC)")
        #to do: person coefficent evaluation
        #前面y_pred也不能用softmax
    else :
        y_preds = []
        for x_gex ,x_smiles, y_batch in dataloader:
            ...
            with torch.no_grad():
                y_truths = np.concatenate([y_truths, y_batch.cpu().detach().numpy().ravel()])
                # y_pred = torch.sigmoid(classifier(x_smiles,x_gex)).detach() #之前是二分类，可以直接调用sigmoid转到01区间就是，反正最后是一个rank
                y_pred = nn.functional.softmax(classifier(x_smiles,x_gex),dim=1).detach() #多分类的时候需要可loss_fn统一，所以调用softmax得到每个分类下的概率值，但目前并不支持回归任务
                y_preds.append( y_pred.cpu().detach().tolist() )
        y_preds = [token for st in y_preds for token in st]
        y_preds = np.array(y_preds)
        ...
        if class_num == 2:
            ...
            if test_flag :#做zero shot
                ...
                #暂时就是分类本身，不考虑概率（该分类概率和其他分类概率）
            else:
                y_preds = y_preds
                #多分类AUROC怎么评估，输入什么
        else:
            raise Exception("只能是正整数")
...
#pdr_dataloader按照【一个病人，所有药】的顺序输出，最后输出行为病人，列为200+药物的评分矩阵
