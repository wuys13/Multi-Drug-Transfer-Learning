...
def train_ae(s_dataloaders, t_dataloaders, **kwargs):
    ...
    if kwargs['retrain_flag']:
        ...
        try:
            autoencoder.load_state_dict(torch.load(os.path.join(kwargs['model_save_folder'], 'ae.pt')))
        # except:
            loaded_model = torch.load(os.path.join(kwargs['model_save_folder'], 'ae.pt'))
            # new_loaded_model = {key: val for key, val in loaded_model.items() if key in autoencoder.state_dict()}
            ...
            # corrected_model = OrderedDict({key: new_loaded_model[key] for key in autoencoder.state_dict()})
            # autoencoder.load_state_dict(corrected_model)
