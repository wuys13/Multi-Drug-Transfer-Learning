#根据最近的AUC/drug文件夹中生成的模型（每一折各有一个最优化的），该模型是训好的share_encoder+decoder
#故如果60组超参都跑完了，理论上就是最后一组（300,3000,0.1）的每折上最佳模型
#想要正常输出最佳模型，首先要先判断哪一组最好，然后no-train下（不用再训两个dsnae模型）跑完这一组
#更新了AUC/drug/target_classifier_{0-4}的模型之后，这个reproduce脚本就会直接用这个finetune好的模型去复现target domain的结果
import pandas as pd
import torch
import json
import os
