...
def coral_ae_train_step(ae, s_batch, t_batch, device, optimizer, alpha, history, scheduler=None):
    ...
    # s_cov = cov(s_code).detach() #能不能这样？？？？？？
    # t_cov = cov(t_code).detach()

    coral_loss = (torch.square(torch.norm(cov(s_code) - cov(t_code), p='fro'))) / (4 * (s_code.size()[-1] ** 2))
    # coral_loss = (torch.square(torch.Tensor(1).to(device))) / (4 * (s_code.size()[-1] ** 2))
